...
export const POST: APIRoute = async ({ params, request, clientAddress }) => {
  ...
  if (!PermissionManager.hasPermission(session.role, 'rules:disable')) {
    return createErrorResponse('FORBIDDEN', 'Your role lacks permission to disable rules.', 403);
  }

  const { id } = params;
  if (!id) return createErrorResponse('BAD_REQUEST', 'Rule ID required.', 400);

  const rule = AdminStore.disableRule(id, session.email);
  if (!rule) {
  ...
  AuditLogger.recordEvent({
    ...
    eventType: 'RULE_EMERGENCY_DISABLED',
